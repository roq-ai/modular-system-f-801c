...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.business_application?.length > 0) {
      const create_business_application = body.business_application;
      body.business_application = {
        create: create_business_application,
      };
    } else {
      delete body.business_application;
    }
