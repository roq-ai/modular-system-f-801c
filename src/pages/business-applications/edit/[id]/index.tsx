...
import { getBusinessApplicationById, updateBusinessApplicationById } from 'apiSdk/business-applications';
...
function BusinessApplicationEditPage() {
  ...
  const handleSubmit = async (values: BusinessApplicationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBusinessApplicationById(id, values);
      mutate(updated);
      resetForm();
