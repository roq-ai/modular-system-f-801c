...
import { getBusinessApplicationById } from 'apiSdk/business-applications';
...
function BusinessApplicationViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BusinessApplicationInterface>(
    () => (id ? `/business-applications/${id}` : null),
    () =>
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Business Application Detail View
          </Text>
          {hasAccess('business_application', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/business-applications/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('feedback', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/feedbacks/create?business_application_id=${data?.id}`}>
